...
# Oxytocin model: all 4 institutes, baseline
...
# Oxytocin model: all 2 institutes, violation of expectations

Analysis includes pre- and post-samples in both positive and negative context of a subset, only AU and IRTA.

## Descriptive

```{r}
dat |> 
  dplyr::filter(violation_expectations == "yes") |> 
  group_by(context, sample) |> 
  get_summary_stats(oxyt_pgmL, show=c("median","q1","q3","iqr")) |> 
  kable(caption = "Oxytocin data") |>
  kable_styling(bootstrap_options = c("striped", "hover"), font_size = 12) |> 
...
Model with 1 interaction of fixed effects and random intercepts but constant slope.
...
mod.oxyt <- lmerTest::lmer(log(oxyt_pgmL) ~
                             ...
                             (1|pig_group_unique) + 
                             (1|pig_id_unique),
                           data    = dat |> dplyr::filter(violation_expectations == "yes"),
                           REML    = FALSE,
                           control = contr)
```
...
## Suppl. Figure 2: Log-Oxytocin all 4 institutes
...
## Figure 7: Oxytocin (violating expectations) AU+IRTA
...
plot <- dat |> 
  ...
  geom_point(aes(x = jit, 
                 ...
                 shape = institute), 
             size = 2.5, alpha = 0.7) + 
  scale_colour_manual(values = c("#481568FF", "#94D840FF")) +
...
```{r}
png("./plots/oxyt-viol-exp-context-type.png",
    width = 200, height = 180, units = "mm",
    pointsize = 10, res = 600)
